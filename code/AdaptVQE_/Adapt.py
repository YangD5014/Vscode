...
class AdaptVQE_Y(object):
    ...
    def get_operator_component(self)->List[List[Instruction]]:
        if len(self.opertor_pool_1)!= 0:
            temp = self.opertor_pool_1
            temp.extend(self.opertor_pool_2)
            self.opertor_component = EvolvedOperatorAnsatz(operators=temp,parameter_prefix='sd',insert_barriers=True).to_instruction()
            self.opertor_instruction  = self.Instruction_topart(instruction=self.opertor_component)
    


    def Instruction_topart(cls,instruction:Instruction):
